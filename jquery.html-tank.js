/**
 * @overview A jQuery plugin for turning any HTML element into a movable tank.
 * @requires jquery
...
;(function($) {
	 ...
	 */
	$.fn.htmlTank = function (options) {
		// One tank at a time. If tank has already been created, do not create a new one.
		if($("#htmlTankCustomTank").length !==  0) {
			return this;
		}

		var thisElm = this, // Used in the unhtmlTank function.
		...
		// Handles size changes with elements that have no width defined, or that have it defined with %.
		...
		$fullTank = $("<div></div>").attr("id", "htmlTankCustomTank");
		$tankBody = this.clone()
			.css({
					"margin": "0",
					"width": widString,
		 ...
		 * @description 
		 *
		 * Handles mousepressed events. Controls movement of the tank depending on if left, right, or middle mouse button is clicked.
		 * Note, there is no button on the mouse to make the tank go backward, or to fire the weapon; these actions require the keyboard.
		 *
		 ...
		 * @description
		 *
		 * Shoots a bullet from the tank gun, if there isn't already one present (from the latest shot).
		 * This function can only be called with the keyboard (not the mouse) - user presses ENTER to call it.
		 */
		function fireWeapon() {
			// Remove current bullet if user fires again.
			if(weaponFired) {
				$("#htmlTankBullet").remove();
				bulletR = 0;
			}
		 ...
		 * @description
		 *
		 * Moves tank backward, setting both wheels backward as well.
		 * This function can only be called with the keyboard (not the mouse).
		 *
		 */
		...
		// Blocks default right-click functionality
